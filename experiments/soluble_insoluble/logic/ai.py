...
def train_model(
    ...
    """
    Train ML model on generated teacher dataset.
    """
    # Prepare data
    X = []
    y = []
    ...
    model.save(Path(MODEL_CONFIG["tensorboard_logdir"]) / "model.keras")
...
def predict(
    ...
    """
    Predict compact parameters from input.
    """
...
def params_to_timeline(
    ...
    """
    Convert predicted parameters into time-series curves.
    """

    A = predicted_params.get("dissolved_fraction", 0.0)
    k = predicted_params.get("solution_concentration", 0.0)
    ...
    t = 0.0
    while t <= total_time:
        value = A * (1 - np.exp(-k * t))
        value = float(np.clip(value, 0.0, 1.0))

        times.append(t)
        dissolved_fraction.append(value)
